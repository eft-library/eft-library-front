...
파티 UI는 모든 사용자에게 표시한다. 로그인 사용자는 일반 계정 토큰으로 방 생성·입장·자동 복원·WebSocket 연결을 사용하고, 비로그인 사용자는 파티 창에서 로그인 안내를 확인한다.
...
- 참여 방은 지도별 키가 아닌 계정별 키로 저장한다. 라이브 맵 경로가 바뀌어도 파티 WebSocket을 유지한다.
- 방 목록은 현재 지도 `map_id`로 필터링하지 않는다. 어느 지도에서 만든 방이든 모든 라이브 맵에서 검색하고 입장할 수 있다.
- 기존 라이브 맵의 지도·층 선택이 바뀌면 `view_map`을 전송한다. 파티 창에 별도의 지도 선택 UI를 두지 않는다.
- 멤버 목록에는 서버가 보낸 현재 언어의 지도명·층명을 표시한다.
- 실시간 위치는 `map_id`와 `floor_id`가 현재 화면과 모두 일치할 때만 지도에 그린다. 지도 전환으로 기존 위치의 소속을 변경하지 않는다.
- 새 `send_location` 위치는 수신 시점에 사용자가 보고 있는 라이브 맵을 위치의 지도로 사용한다. 좌표로 층을 판정해 원본 `yaw`와 함께 전송한다.
- 영구 공유 마커와 파티원 위치는 현재 `map_id`가 맞으면 층과 관계없이 선명하게 표시한다. 각 마커 위에는 해당 층 이름을 항상 표시한다.
- 지도를 전환하면 이전 지도 마커를 최신 `version`으로 순차 삭제한다. 방장은 이전 지도의 모든 마커를, 일반 멤버는 자신이 만든 마커만 삭제한다. 이동한 지도에서는 새 마커를 바로 생성할 수 있다.
