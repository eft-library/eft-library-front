...
import { ContainerViewTypes } from "./container.types";
import ContainerTable from "./ContainerTable/container-table";
import { useState } from "react";
import { Input } from "@/components/ui/input";
import { Search } from "lucide-react";

export default function ContainerView({ containerList }: ContainerViewTypes) {
    ...
    <div className="min-h-screen dark:bg-[#1e2124] dark:text-white bg-gray-50 text-black">
      <div className="container mx-auto px-4 py-4 sm:py-8 space-y-6 sm:space-y-8 max-w-6xl">
        <div className="text-center mb-6 md:mb-8">
          ...
          <ContainerTable containerList={containerList} word={word} />
        </div>
      </div>
    </div>
  );
