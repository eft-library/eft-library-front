...
import { BackpackViewTypes } from "./backpack.types";
import BackpackTable from "./BackpackTable/backpack-table";
import { useState } from "react";
import { Input } from "@/components/ui/input";
import { Search } from "lucide-react";

export default function BackpackView({ backpackList }: BackpackViewTypes) {
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-4 sm:py-8 space-y-6 sm:space-y-8 max-w-6xl">
        <div className="text-center mb-6 md:mb-8">
          ...
          <BackpackTable backpackList={backpackList} word={word} />
        </div>
      </div>
    </div>
  );
