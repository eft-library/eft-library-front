...
import type { FaceCoverViewTypes } from "./face-cover.types";
import FaceCoverTable from "./FaceCoverTable/face-cover-table";
import { useState } from "react";
import { Input } from "@/components/ui/input";
import { Search } from "lucide-react";

export default function FaceCoverView({ face_cover_data }: FaceCoverViewTypes) {
  ...
  return (
    <div className="min-h-screen dark:bg-[#1e2124] dark:text-white bg-gray-50 text-black">
      <div className="container mx-auto px-4 py-4 sm:py-8 space-y-6 sm:space-y-8 max-w-6xl">
        ...
        <FaceCoverTable face_cover_data={face_cover_data} word={word} />
      </div>
    </div>
  );
}
