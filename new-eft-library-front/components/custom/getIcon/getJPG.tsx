...
type AllColorKeys = keyof typeof ALL_COLOR;

interface DynamicJPG {
...
export const ItemJPG = ({ x, y, svgValue, scale }: DynamicJPG) => {
  if (!(svgValue in jpgComponents)) {
    return null;
  }

  const Jpg = jpgComponents[svgValue as keyof typeof jpgComponents];
