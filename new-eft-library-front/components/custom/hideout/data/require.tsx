    ...
    return (items.length > 0 && (
        <>
            {items.map((item, index) => (
                <div key={"id" in item && item.id || index} className={"flex items-center"}>
                    {item.image && (
                        <div
                            className={"flex items-center justify-center cursor-pointer"}
                        >
                            <Gallery>
                                <Item original={ type === "skill" ? formatImage(item.image) : item.image} width="400" height="380">
                                    {({ref, open}) => (
                                        <Image
                                            ref={ref}
                                            onClick={open}
                                            src={ type === "skill" ? formatImage(item.image || "") : item.image || ""}
                                            height={80}
                                            width={80}
                    ...
                    {(type === "trader" || type === "skill") && <div className={"mr-[2px]"}/>}
                    <span className={"text-center font-bold text-lg"}>{item.name_kr}</span>
