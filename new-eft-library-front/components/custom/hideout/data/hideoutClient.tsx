...
export default function HideoutClient({hideoutList}:HideoutClient) {
    ...
    return <div className="w-full">
        {hideoutList.map((hideout) => checkViewHideout(hideout.master_id) && hideout.data.map((info) => <div key={info.level_id}>
            <div id={info.level_id} className={"flex justify-center items-center w-full flex-col"}>
                <div className={"flex w-full mb-[2px] mt-[2px]"}>
                    <span
                        className="text-center font-bold text-xl">{hideout.master_name_kr} {info.level_info[0].level}</span>
                </div>
                <div
                    id={info.level_id}
