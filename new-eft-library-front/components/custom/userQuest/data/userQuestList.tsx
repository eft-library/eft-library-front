...
import type {UserQuestList, UserQuestInfo} from "@/components/custom/userQuest/data/userQuestType";
import Link from "next/link";

export default function UserQuestList({userQuest,successQuest,checkedQuest,checkedBox}: UserQuestList) {
        ...
        return (
            ...
            >
                <Link href={`/quest/detail/${item.quest_id}`} target="_blank">
                    <Text fontWeight={600}>{item.quest_name_kr
                        .substring(0, item.quest_name_kr.indexOf("("))
                        .trim()}</Text>
                </Link>
                <Link href={`/quest/detail/${item.quest_id}`} target="_blank">
