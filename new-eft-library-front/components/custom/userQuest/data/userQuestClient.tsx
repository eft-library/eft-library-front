...
import type{UserQuest, UserClientQuest, FetchSchema} from "@/components/custom/userQuest/data/userQuestType";
...
export default function UserQuestClient({userQuestList}: UserClientQuest) {
    ...
    return <div className={"w-full h-full"}>
        {userQuest.length < 0 || !userQuest[0].npc_id ?
            (<span className={"text-white font-bold text-lg"}>퀘스트 플래너에 등록할 퀘스트를 검색하여 추가 버튼을 눌러주세요.</span>)
            : <div>
                {/* 상단 전체 선택 및 삭제 버튼 */}
                        ...
                        <div key={npc.npc_id} className="border-b-0">
                            ...
                            >
                                <div className={"flex gap-2 items-center"}>
                                    <Image src={formatImage(npc.npc_image)} alt={npc.npc_name_en} width={40} height={40} className={"rounded-lg"}/>
                                    <span className="flex-1 text-left font-bold">{npc.npc_name_kr}</span>
                                </div>
                                <svg
