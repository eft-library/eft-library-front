// 탈출구 아이콘
import { ITEM_COLOR } from 'src/utils/colorConstants';
const ScavExtraction = (props) => {
  return (
    <svg
      version="1.1"
      id="Layer_1"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      x="0px"
      y="0px"
      width={props.width ? props.width : '20px'}
      height={props.height ? props.height : '20px'}
      viewBox="0 0 22 32"
      style={{ enableBackground: 'new 0 0 22 32' }}
      xmlSpace="preserve"
    >
      <style type="text/css">{`.st0{fill:${props.border ? props.border : '#010101'};}.st1{fill:${props.color ? props.color : ITEM_COLOR.SCAV_EXTRACTION};}`}</style>
      <g>
        <path className="st0" d="M21,1v30H1V1H21 M22,0H0v32h22V0L22,0z" />
      </g>
      <g>
        <path
          className="st0"
          d="M21,1.26v29.47L7,27.44V4.56L21,1.26 M22,0L6,3.76v24.47L22,32V0L22,0z"
        />
      </g>
      <g>
        <path
          className="st0"
          d="M21,1.26v29.47L7,27.44V4.56L21,1.26 M22,0L6,3.76v24.47L22,32V0L22,0z"
        />
      </g>
      <g>
        <path
          className="st0"
          d="M21,1.26v29.47L7,27.44V4.56L21,1.26 M22,0L6,3.76v24.47L22,32V0L22,0z"
        />
      </g>
      <g>
        <path
          className="st0"
          d="M21,1.26v29.47L7,27.44V4.56L21,1.26 M22,0L6,3.76v24.47L22,32V0L22,0z"
        />
      </g>
      <g>
        <path
          className="st0"
          d="M21,1.26v29.47L7,27.44V4.56L21,1.26 M22,0L6,3.76v24.47L22,32V0L22,0z"
        />
      </g>
      <g>
        <path
          className="st0"
          d="M21,1.26v29.47L7,27.44V4.56L21,1.26 M22,0L6,3.76v24.47L22,32V0L22,0z"
        />
      </g>
      <g>
        <polygon className="st1" points="22,0 6,3.76 6,28.24 22,32 	" />
      </g>
      <g>
        <path
          className="st0"
          d="M21,1.26v29.47L7,27.44V4.56L21,1.26 M22,0L6,3.76v24.47L22,32V0L22,0z"
        />
      </g>
      <path
        className="st0"
        d="M21,1v29.74L7,27.44V4.56l14-3.29 M22,0L6,3.76v24.47L22,32V0L22,0z"
      />
      <g>
        <path
          className="st0"
          d="M9,15c-0.55,0-1,0.45-1,1c0,0.55,0.45,1,1,1c0.55,0,1-0.45,1-1C10,15.45,9.55,15,9,15L9,15z"
        />
      </g>
    </svg>
  );
};

export default ScavExtraction;
