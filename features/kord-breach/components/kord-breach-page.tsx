...
export function KordBreachPage({ modifiers, locale }: { modifiers: KordBreachModifiersResponse; locale: Locale }) {
  ...
  return (
    <main className="min-h-[calc(100vh-4rem)] px-4 py-6 sm:px-6 lg:px-8">
      ...
      {presetOpen && <PresetDialog presets={presets} selected={[...selected]} accessToken={session?.accessToken} authLoading={status === "loading"} busy={busy} setBusy={setBusy} onClose={() => setPresetOpen(false)} onLoad={loadPreset} onChange={setPresets} />}
    </main>
  );
}
...
function StatusPanel({ t, positives, negatives, total, busy, onRemove, onRandom, onPresets, onReset }: { t: typeof copy.ko | typeof copy.en | typeof copy.ja; positives: KordBreachModifier[]; negatives: KordBreachModifier[]; total: number; busy: boolean; onRemove: (id: string) => void; onRandom: () => void; onPresets: () => void; onReset: () => void }) {
  return <section aria-label={t.board} className="rounded-xl border border-line bg-surface p-4 shadow-sm sm:sticky sm:top-20 sm:z-20 sm:max-h-[calc(100vh-6rem)] sm:overflow-y-auto sm:p-5 sm:shadow-lg">
...
function ModifierCard({ item, locale, isGlobal, selected, disabled, onClick }: { item: KordBreachModifier; locale: Locale; isGlobal: boolean; selected: boolean; disabled: boolean; onClick?: () => void }) { const name = locale === "ja" ? item.nameJa ?? item.nameEn : locale === "en" ? item.nameEn : item.nameKo; const effect = locale === "ja" ? item.effectJa ?? item.effect : locale === "en" ? item.effectEn ?? item.effect : item.effectKo ?? item.effect; const tone = item.category === "positive" ? "emerald" : item.category === "negative" ? "red" : "orange"; return <button type="button" onClick={onClick} disabled={isGlobal || disabled} aria-pressed={isGlobal ? undefined : selected} title={disabled ? "선택한 특성과 함께 사용할 수 없습니다." : effect} className={`relative flex min-h-48 flex-col items-center overflow-hidden rounded-lg border p-3 text-center outline-none transition duration-200 hover:-translate-y-0.5 focus-visible:ring-2 focus-visible:ring-orange-500 disabled:hover:translate-y-0 ${selected ? tone === "emerald" ? "-translate-y-0.5 border-emerald-500 bg-emerald-100/80 shadow-lg shadow-emerald-500/20 ring-2 ring-emerald-500/35 dark:bg-emerald-950/60" : "-translate-y-0.5 border-red-500 bg-red-100/80 shadow-lg shadow-red-500/20 ring-2 ring-red-500/35 dark:bg-red-950/60" : "border-line bg-surface-elevated hover:border-gray-400"} ${disabled ? "cursor-not-allowed opacity-35 grayscale" : ""}`}>
    ...
    <div className={`my-1 flex size-20 items-center justify-center rounded-full transition duration-200 ${selected ? tone === "emerald" ? "bg-emerald-500/15 shadow-inner ring-1 ring-emerald-500/20" : "bg-red-500/15 shadow-inner ring-1 ring-red-500/20" : ""}`}><Image src={item.iconUrl} alt="" width={64} height={64} className={`size-16 object-contain transition duration-200 dark:brightness-110 ${selected ? "scale-110 drop-shadow-md" : ""}`} /></div><strong className={`text-xs transition-colors ${selected ? tone === "emerald" ? "text-emerald-900 dark:text-emerald-200" : "text-red-900 dark:text-red-200" : ""}`}>{name}</strong><span className={`mt-0.5 text-xs leading-4 ${selected ? tone === "emerald" ? "text-emerald-700 dark:text-emerald-300/80" : "text-red-700 dark:text-red-300/80" : "text-gray-500 dark:text-gray-400"}`}>{item.nameEn}</span><p className={`mt-2 line-clamp-3 text-xs leading-5 ${selected ? "text-gray-700 dark:text-gray-200" : "text-gray-600 dark:text-gray-300"}`}>{effect}</p>
  </button>; }

function PresetDialog({ presets, selected, accessToken, authLoading, busy, setBusy, onClose, onLoad, onChange }: { presets: KordBreachPreset[]; selected: string[]; accessToken?: string; authLoading: boolean; busy: boolean; setBusy: (value: boolean) => void; onClose: () => void; onLoad: (preset: KordBreachPreset) => void; onChange: (presets: KordBreachPreset[]) => void }) {
  const [dialogMessage, setDialogMessage] = useState("");
  const bySlot = new Map(presets.map((preset) => [preset.slotNo, preset]));
  async function save(slotNo: number) { if (!accessToken) return; setBusy(true); setDialogMessage(""); try { const result = await saveKordBreachPreset(slotNo, `프리셋 ${slotNo}`, selected, accessToken); if (result.success) onChange([...presets.filter((p) => p.slotNo !== slotNo), result.preset].sort((a, b) => a.slotNo - b.slotNo)); else setDialogMessage(saveError(result.reason)); } catch { setDialogMessage("프리셋 저장에 실패했습니다."); } finally { setBusy(false); } }
  async function remove(slotNo: number) { if (!accessToken) return; setBusy(true); setDialogMessage(""); try { const result = await deleteKordBreachPreset(slotNo, accessToken); if (result.success) onChange(presets.filter((p) => p.slotNo !== slotNo)); } catch { setDialogMessage("프리셋 삭제에 실패했습니다."); } finally { setBusy(false); } }
